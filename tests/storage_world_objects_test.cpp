...
return &(*it);
        ...
        class EmptySerializationChar : public CChar
        {
        public:
                void r_Write( CScript & ) override
                {
                        // Suppress serialized output so SaveWorldObject treats this
                        // character as skipped when the save parity matches.
                }
        };
}
...
TEST_CASE( TestSavingContainedItemPersistsAncestorMetaWhenSerializationSkipped )
{
        StorageServiceFacade storage;
        if ( !storage.Connect())
        {
        ...
        EmptySerializationChar character;
        character.SetUID( 0x28u );
        character.SetBaseID( 0x190 );
        character.SetName( "Paragon" );
        character.SetTopLevel( true );
        character.SetTopPoint( CPointMap( 1500, 1600, 10 ));
        character.SetTopLevelObj( &character );

        CItem potion;
        potion.SetUID( 0x040001000u );
        potion.SetBaseID( 0x0f0f );
        potion.SetName( "refresh potion" );
        potion.SetTopLevel( false );
        potion.SetTopLevelObj( &character );

        character.AddContent( &potion );

        if ( !storage.Service().SaveWorldObject( &potion ))
        {
                throw std::runtime_error( "Saving contained item failed" );
        }

        const std::string characterUid = std::to_string( static_cast<unsigned long long>( character.GetUID() ));
        const std::string potionUid = std::to_string( static_cast<unsigned long long>( potion.GetUID() ));

        bool foundCharacterMeta = false;
        bool foundCharacterData = false;
        bool foundPotionMeta = false;
        ...
        for ( const auto & stmt : statements )
        {
                if ( stmt.query.find( "`test_world_objects`" ) != std::string::npos )
                {
                        ...
                        {
                                if ( stmt.parameters[0] == characterUid )
                                {
                                        foundCharacterMeta = true;
                                }
                                else if ( stmt.parameters[0] == potionUid )
                                {
                                        foundPotionMeta = true;
                                }
                        }
                }
                else if ( stmt.query.find( "`test_world_object_data`" ) != std::string::npos &&
                        stmt.parameters.size() >= 1 &&
                        stmt.parameters[0] == characterUid )
                {
                        foundCharacterData = true;
                }
        }

        if ( !foundCharacterMeta )
        {
                throw std::runtime_error( "Ancestor metadata upsert was skipped" );
        }

        if ( foundCharacterData )
        {
                throw std::runtime_error( "Skipped serialization should not rewrite character data" );
        }
