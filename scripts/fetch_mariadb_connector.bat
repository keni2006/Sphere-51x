...
set "WIN_PAYLOAD="
for /f "usebackq delims=" %%I in (`powershell -NoProfile -Command "Get-ChildItem -Directory -Recurse -Path '%WIN_TEMP%' | Where-Object { Test-Path (Join-Path $_.FullName 'include\\mysql.h') -and (Test-Path (Join-Path $_.FullName 'lib\\libmariadb.lib')) } | Select-Object -First 1 -ExpandProperty FullName"`) do set "WIN_PAYLOAD=%%I"
...
set "LINUX_PAYLOAD="
for /f "usebackq delims=" %%I in (`powershell -NoProfile -Command "Get-ChildItem -Directory -Recurse -Path '%LINUX_TEMP%' | Where-Object { Test-Path (Join-Path $_.FullName 'include/mariadb/mysql.h') -or Test-Path (Join-Path $_.FullName 'include/mysql/mysql.h') -or Test-Path (Join-Path $_.FullName 'include/mysql.h') } | Select-Object -First 1 -ExpandProperty FullName"`) do set "LINUX_PAYLOAD=%%I"
