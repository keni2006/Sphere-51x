...
powershell -NoProfile -ExecutionPolicy Bypass -Command "param([string]$Uri,[string]$OutFile) if (Test-Path -LiteralPath $OutFile) { return }; $ProgressPreference='SilentlyContinue'; Invoke-WebRequest -Uri $Uri -OutFile $OutFile -UseBasicParsing" "%WIN_URL%" "%WIN_MSI%"
if errorlevel 1 (
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "param([string]$Uri,[string]$OutFile) if (Test-Path -LiteralPath $OutFile) { return }; $ProgressPreference='SilentlyContinue'; Invoke-WebRequest -Uri $Uri -OutFile $OutFile -UseBasicParsing" "%LINUX_URL%" "%LINUX_TARBALL%"
if errorlevel 1 (
