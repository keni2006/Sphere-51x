...
int CServer::PrintPercent( long iCount, long iTotal )
{
        ...
        static const char * const kWaveFrames[] =
        {
                "[~    ]",
                "[ ~   ]",
                "[  ~  ]",
        ...
        const char * frame = kWaveFrames[frameIndex];
        frameIndex = (frameIndex + 1) % COUNTOF(kWaveFrames);
        CGString sProgress;
        int len = sProgress.Format( "%s %3d%%", frame, iPercent );
        PrintStr( sProgress );
        while ( len-- )
        {
                PrintStr( "\x08" );
